...
import com.arbitrage.service.validators.market.MarketRulesService;
...
public class SignalValidatorImpl implements SignalValidator {
  ...
  private final MarketRulesService marketRulesService;
  ...
  public BalanceValidationReportDto validateBalance(SignalContext ctx) {
    ...
    SizingResultDto sizing = executionSizingService.sizeForBalances(resolved, availableByBucket);
    ...
    if (sizing.isScaled()) {
      log.info(
          ...
          resolved.get(0).getQty(),
          sizing.getExecQty().get(0),
          resolved.get(1).getQty(),
          sizing.getExecQty().get(1));
    } else {
      log.info(
  ...
  @Override
  public MarketValidationReportDto validateMarket(
      SignalContext ctx, ExecutionPlanDto planFromBalance) {
    return marketRulesService.applyAll(ctx, planFromBalance);
  }

  // Build plan with custom per-leg execQty (used after sizing)
  private ExecutionPlanDto buildPlan(
      List<ResolvedLegDto> resolved, List<BigDecimal> execQtyOverride) {
    ExecutionPlanDto.ExecutionPlanDtoBuilder pb = ExecutionPlanDto.builder();
    for (int i = 0; i < resolved.size(); i++) {
      ResolvedLegDto rl = resolved.get(i);
      BigDecimal execQty =
          (execQtyOverride != null && i < execQtyOverride.size())
