...
/**
 * Immutable processing context passed across validation steps.
 * Carries:
 *  - The original normalized DTO
 *  - A stable 'now' timestamp (injected from Clock for testability)
 *  - The saved DB id (after Phase-0 persist)
 *  - An extensible attributes bag for cross-step hints (optional)
 */
@Value
...
public class SignalContext {
    ...
    /**
     * Factory method to create context using an injected Clock.
     * Keeps 'now' stable for the entire processing pipeline.
     */
    public static SignalContext of(SignalMessageDto dto, Clock clock, UUID savedId) {
        return SignalContext.builder()
                .dto(dto)
                .now(clock.instant())
                .savedSignalId(savedId)
                .build();
    }
    ...
    /**
     * Create a shallowly updated copy with a new attribute.
     * This preserves immutability while enabling step-to-step enrichment.
     */
