...
/**
 * Immutable step outcome for a single validation step.
 * - ok=true  => step passed
 * - ok=false => step failed and a Rejection is provided
 * Keep this class minimal and let Rejection carry details.
 */
@Getter
@Builder
@AllArgsConstructor
public class StepResult {
    ...
    public static StepResult fail(Rejection rejection) {
        return StepResult.builder()
                .ok(false)
                .rejection(Optional.ofNullable(rejection))
                .build();
    }
}
